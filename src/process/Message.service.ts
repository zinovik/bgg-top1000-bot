...
export class MessageService implements ProcessService {
    formatMessage({
        ...
        const newGamesString = this.getAdditionalList(
            '🆕 Game(s) new in Top 1000',
            newGames
        );
        const droppedGamesString = this.getAdditionalList(
            '❌ Game(s) dropped out of Top 1000',
            droppedGames
        );
        ...
        const increasedGamesStrings = Object.keys(increasedGames)
            ...
            .map((change) =>
                this.getAdditionalList(`⬆️ ${change}`, increasedGames[change])
            );
    ...
    private formatGame(
        game: { rank: number; name: string; year: string },
        changeString = ''
    ): string {
